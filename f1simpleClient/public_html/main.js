...
function creaOPTIONSdaJson(objSelect, nomeCampoValue, nomeCampoText, jsonArray) {
	...
	$.each(jsonArray, function() { // per ogni oggetto json dell'array
		$.each(this, function(prop, val) { // per ogni propriet‡ dell'oggetto
			if (nomeCampoValue.indexOf(prop) > -1) { // se la propriet‡ Ë presente nell'elenco dei campi da selezionare
				value =  val;
			};
			if (nomeCampoText.indexOf(prop) > -1) { // se la propriet‡ Ë presente nell'elenco dei campi da selezionare
				text = val;
			};
		});
